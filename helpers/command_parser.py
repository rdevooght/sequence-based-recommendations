...
def predictor_command_parser(parser):
	parser.add_argument('-m', dest='method', choices=['RNN', 'SDA', 'BPRMF', 'FPMC', 'LTM', 'UKNN', 'MM', 'POP'],
	...
	parser.add_argument('--max_length', help='Maximum length of sequences during training (for RNNs)', default=200, type=int)
...
def get_predictor(args):
	...
	if args.method == "MF":
		...
		if args.loss == 'CCE':
			...
			return RNNSampling(interactions_are_unique=(not args.repeated_interactions), loss_function=args.loss, diversity_bias=args.diversity_bias, sampling=args.sampling, recurrent_layer=recurrent_layer, max_length=args.max_length, updater=updater, target_selection=target_selection, sequence_noise=sequence_noise, use_ratings_features=args.rf, use_movies_features=args.mf, use_users_features=args.uf, batch_size=args.batch_size)
		else:
