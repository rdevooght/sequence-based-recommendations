...
class BlackoutLayer(DenseLayer):
	...
	def get_output_for(self, input, deterministic=False, targets=None, **kwargs):
		if input.ndim > 2:
...
# class SparseInputLayer(InputLayer):
...
#             input_var = sparse.csr_matrix(var_name, dtype=theano.config.floatX)
#         else:
...
# class LSTMLayerSparseInput(MergeLayer):
#     r"""
#     lasagne.layers.recurrent.LSTMLayer(incoming, num_units,
#     ingate=lasagne.layers.Gate(), forgetgate=lasagne.layers.Gate(),
#     cell=lasagne.layers.Gate(
...
#         # Initialize parent layer
#         super(LSTMLayerSparseInput, self).__init__(incomings, **kwargs)
...
#             # W_in_stacked is (n_features, 4*num_units). input is then
#             # (n_time_steps, n_batch, 4*num_units).
#             input = sparse.dot(input, W_in_stacked) + b_stacked
...
#         def step(input_n, cell_previous, hid_previous, *args):
#             if not self.precompute_input:
#                 input_n = sparse.dot(input_n, W_in_stacked) + b_stacked
